...
class ThreeTenColor implements ThreeTenAlg {
	...
	/**
	 * Set the next max node to become highlighted.
	 */
	public void highlightNextMax(){
		...
		GraphNode max = queue.element();

		max.setColor(COLOR_HIGHLIGHT);

	}

	/**
	 * Find the max node in the queue.
	 * @return GraphNode
	 */
	public GraphNode findMax(){
		...
		// to use, especially the interface Dequeue.
		//
		...
		GraphNode max = queue.remove();
		stack.addFirst(max);
		max.setColor(COLOR_INACTIVE_NODE);
		max.unsetActive();
		for(GraphEdge e : graph.getIncidentEdges(max)) {
	...
	/**
	 * Update the neighbors now that this (max) node is inactive.
	 * @param maxNode - the node returned from findMax()
	 */
	public void updateNeighborCost(GraphNode maxNode){
		...
		if(maxNode.isActive()==false) {
			for(GraphNode n : graph.getNeighbors(maxNode)) {
				if(n.isActive())
	...
	/**
	 * Depending on where the node is located on the graph, select the colors wisely.
	...
	public Color chooseColor(GraphNode node){
		...
		LinkedList<Color> ourColors = new LinkedList<Color>();
		for(Color c: COLORS) {
			ourColors.add(c);
		}//store all possible node colors into the linked list
		for(GraphNode n : graph.getNeighbors(node)) {
			ourColors.remove(n.color);
		}//remove the colors matching the node's neighbor color
		
		int index=0;
		for(int i=0; i<COLORS.length; i++) {
			if(COLORS[i]==ourColors.getFirst()) {
				index = i;
				break;
			}
		}//get the index from the original colors array
		
		for(GraphNode d: graph.getNeighbors(node)) {
			d.setNbrColor(index);
		}//set all the node's neighbors to regard it as the newly attained index color

		return ourColors.getFirst();//use the lowest indexed color available from the remaining colors
	}
	
	/**
	 * Updates the current color of node with newColor.
	...
	public void updateColor(GraphNode node, Color newColor){
		...
		for(GraphEdge e : graph.getIncidentEdges(node)) {
			if(e.color==COLOR_INACTIVE_EDGE||e.color==COLOR_NONE_EDGE) {
				e.setColor(node.color);
			}
		}
